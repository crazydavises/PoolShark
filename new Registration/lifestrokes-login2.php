...
function login_header($title = 'Log In', $message = '', $wp_error = '') {
	...
	if ( 0 && $shake_error_codes && $wp_error->get_error_code() && in_array( $wp_error->get_error_code(), $shake_error_codes ) )
...
<head>
	...
	<link type="text/css" rel="stylesheet" href="<?php bloginfo('wpurl'); ?>'/wp-content/plugins/poolsharkreg/poolshark_tables.css" />
...
<?php
	...
	echo '		<h1 class="entry-title">Login or Register for LifeStrokes</h1>';
...
function retrieve_password() {
	...
	$message .= '<' . network_site_url("lifestrokes-login2.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . ">\r\n";
 ...
 */
function lifestrokes_register_new_user( $user_email, $pass1, $pass2 ) {
	$errors = new WP_Error();
	...
	if ( $pass1 != $pass2 )
		$errors->add ('mismatched_passwords', __('<strong>ERROR</strong>: Passwords do not match.' ) ); 
...
case 'logout' :
	...
	$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'lifestrokes-login2.php?loggedout=true';
...
case 'retrievepassword' :
	...
	if ($http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'lifestrokes-login2.php?checkemail=confirm';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
...
<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url( 'lifestrokes-login2.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
	<p>
...
<?php if ( 0 && get_option( 'users_can_register' ) ) : ?>
 | <a href="<?php echo esc_url( site_url( 'lifestrokes-login2.php?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a>
<?php endif; ?>
</p>
...
case 'rp' :
	...
	if ( is_wp_error($user) ) {
		wp_redirect( site_url('lifestrokes-login2.php?action=lostpassword&error=invalidkey') );
		exit;
	}
...
?>
<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( site_url( 'lifestrokes-login2.php?action=resetpass&key=' . urlencode( $_GET['key'] ) . '&login=' . urlencode( $_GET['login'] ), 'login_post' ) ); ?>" method="post">
...
case 'register' :
	...
	if ( !get_option('users_can_register') ) {
		wp_redirect( site_url('lifestrokes-login2.php?registration=disabled') );
		exit();
	}
	...
	if ( $http_post ) {
		...
		$pass1 = $_POST['pass1'];
		$pass2 = $_POST['pass2'];
		$errors = lifestrokes_register_new_user($user_email, $pass1, $pass2);
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'lifestrokes-login2.php?checkemail=registered';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
...
<form name="registerform" id="registerform" action="<?php echo esc_url( site_url('lifestrokes-login2.php?action=register', 'login_post') ); ?>" method="post">
	<p>
...
<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'lifestrokes-login2.php', 'login_post' ) ); ?>" method="post">
	<p>
		<label for="user_login"><?php _e('Username') ?><br />
...
<?php elseif ( 0 && get_option('users_can_register') ) : ?>
<a href="<?php echo esc_url( site_url( 'lifestrokes-login2.php?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a> |
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Lost your password?' ); ?></a>
<?php else : ?>
<a href="<?php echo esc_url( wp_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Password Lost and Found' ); ?>"><?php _e( 'Lost your password?' ); ?></a>
