...
case 'logout' :
	...
	$redirect_to = 'lifestrokes-login.php?loggedout=true';
	...
	if ( $_POST ) {
		...
		if ( empty( $errors ) ) {
			...
			if (!$user_email || $user_email != $_POST['user_email']) {
				...
				$message .= get_option('siteurl') . "/lifestrokes-login.php?action=rp&key=$key\r\n";

				if (FALSE == wp_mail($user_email, sprintf(__('[%s] Password Reset'), get_option('blogname')), $message)) {
					die('<p>' . __('The e-mail could not be sent.') . "<br />\n" . __('Possible reason: your host may have disabled the mail() function...') . '</p>');
				} else {
					wp_redirect('lifestrokes-login.php?checkemail=confirm');
					exit();
				}
			}
		}
...
<form name="lostpasswordform" id="lostpasswordform" action="lifestrokes-login.php?action=lostpassword" method="post">
	<p>
...
<?php if (get_option('users_can_register')) : ?>
	<li><a href="<?php bloginfo('wpurl'); ?>/lifestrokes-login.php"><?php _e('Login') ?></a></li>
	<li><a href="<?php bloginfo('wpurl'); ?>/lifestrokes-login.php?action=register"><?php _e('Register') ?></a></li>
...
case 'rp' :
	...
	if ( empty( $key ) ) {
		wp_redirect('lifestrokes-login.php?action=lostpassword&error=invalidkey');
		exit();
	}
	...
	$message .= get_option('siteurl') . "/lifestrokes-login.php\r\n";
	...
	if (FALSE == wp_mail($user->user_email, sprintf(__('[%s] Your new password'), get_option('blogname')), $message)) {
		...
		wp_redirect('lifestrokes-login.php?checkemail=newpass');
		exit();
	}
break;

case 'register' :
	echo "made it to register";
	if ( FALSE == get_option('users_can_register') ) {
		wp_redirect('lifestrokes-login.php?registration=disabled');
		exit();
	}
	...
	if ( $_POST ) {
		...
		if( strpos( " ".$pass1, "\\" ) )
			$errors['pass1'] = __( '<strong>ERROR</strong>: Passwords may not contain the character "\\".' );

		/* checking the password has been typed twice the same */
		if ( $pass1 != $pass2 )
			$errors['pass1'] = __( '<strong>ERROR</strong>: Passwords do not match.' );
		
		if ( empty($pass1) || $pass1 == '')
			$errors['pass1'] = __( '<strong>ERROR</strong>: Please enter a password.' );
...
<form name="registerform" id="registerform" action="lifestrokes-login.php?action=register" method="post">
	<p>
		<label><?php _e('E-mail:') ?><br />
...
<ul>
	<li><a href="<?php bloginfo('wpurl'); ?>/lifestrokes-login.php"><?php _e('Login') ?></a></li>
	<li><a href="<?php bloginfo('wpurl'); ?>/lifestrokes-login.php?action=lostpassword" title="<?php _e('Password Lost and Found') ?>"><?php _e('Lost your password?') ?></a></li>
	<li><a href="<?php bloginfo('url'); ?>/" title="<?php _e('Are you lost?') ?>"><?php printf(__('Back to %s'), get_bloginfo('title')); ?></a></li>
</ul>
...
default:
	...
	elseif	( 'registered' == $_GET['checkemail'] )	$errors['registered']		= __('Registration complete. You may now log in with your new password.');

	login_header(__('Login'));
?>

<form name="loginform" id="loginform" action="lifestrokes-login.php" method="post">
	<p>
		<label><?php _e('Username:') ?><br />
